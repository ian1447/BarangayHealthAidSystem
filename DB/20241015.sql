...
insert  into `child_sub_opt`(`id`,`out_patient_id`,`remarks`,`mode`,`added_by`,`added_on`) values (1,1,'Vitamin A injected into the body.',0,1,'2024-10-15 22:32:59'),(5,1,'asdf',1,1,'2024-10-15 22:36:16'),(6,1,'Naka igit ug 10 ka worms',1,1,'2024-10-15 22:36:25');
...
BEGIN
	...
	SELECT c.*,DATE_FORMAT(c.`added_on`, "%M %d,%Y") `date` FROM `child_sub_opt` c WHERE c.`mode` = _mode AND DATE(c.`added_on`)  = DATE(NOW());
    else 
	SELECT c.*,DATE_FORMAT(c.`added_on`, "%M %d,%Y") `date` FROM `child_sub_opt` c WHERE c.`mode` = _mode AND DATE(c.`added_on`)  != DATE(NOW());
    end if;
    END */$$
